...
!   I_BC: 
    ...
    IMPLICIT NONE
    INTEGER :: NB,INDEX,I_BC(6),M,ORDER
...
!   For: Ax=b      
    ...
    REAL(KIND=DP):: ALFA,RHO,RHO_P,OMEGA,BETA,ERROR
...
!---INITIALIZATION------------------------------------
    ...
    END DO
!--------------------------------------------------
    R=B-MATMUL_2D_1D(A,X,TOTAL_CELL,TOTAL_CELL)
    ...
    DO NC=1,NTOTAL
      RHO_P=RHO
      RHO=MATMUL_1D_1D(R0H,R,TOTAL_CELL)

      BETA=(RHO/RHO_P)/(ALFA/OMEGA)

      P=R+BETA*(P-OMEGA*NU)

      NU=MATMUL_2D_1D(A,P,TOTAL_CELL,TOTAL_CELL)

      ALFA=RHO/MATMUL_1D_1D(R0H,NU,TOTAL_CELL)

      H=X+ALFA*P

      ERROR=RMS(B-MATMUL_2D_1D(A,H,TOTAL_CELL,TOTAL_CELL),TOTAL_CELL)
      ...
      T=MATMUL_2D_1D(A,S,TOTAL_CELL,TOTAL_CELL)

      OMEGA=MATMUL_1D_1D(T,S,TOTAL_CELL)/MATMUL_1D_1D(T,T,TOTAL_CELL)

...
!==========================================================! 
    ...
    IF(ISOURCE.EQ.1)THEN        ! ISOURCE=1: CONSTANT SOURCE
      DO M=1,TOTAL_CELL
        CELL_FV(M)%CELL_FX=CELL_FV(M)%CELL_FX+SOURCE_X
        CELL_FV(M)%CELL_FY=CELL_FV(M)%CELL_FY+SOURCE_Y
        CELL_FV(M)%CELL_FZ=CELL_FV(M)%CELL_FZ+SOURCE_Z
      END DO
      ...
      DO M=1,TOTAL_CELL
        ...
        IF(LOC.GE.LOC_SPG_START.AND.LOC.LE.LOC_SPG_END)THEN
          CELL_FV(M)%CELL_FX=CELL_FV(M)%CELL_FX-  &
                             SITA(LOC,LOC_SPG_START,LS,2)*(CELL_FV(M)%CELL_VEL(1)-USPG)
          CELL_FV(M)%CELL_FY=CELL_FV(M)%CELL_FY-  &
                             SITA(LOC,LOC_SPG_START,LS,2)*(CELL_FV(M)%CELL_VEL(2)-VSPG)
          CELL_FV(M)%CELL_FZ=CELL_FV(M)%CELL_FZ-  &
                             SITA(LOC,LOC_SPG_START,LS,2)*(CELL_FV(M)%CELL_VEL(3)-WSPG)
        END IF
      END DO
    END IF  

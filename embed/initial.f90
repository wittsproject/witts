...
!-----GET THE VALUE ON CELLS
      ...
      DO M=1,TOTAL_CELL
        IF(CELL_FV(M)%CELL_ACTIVE.EQ.1)THEN
          MM=MM+1
          CELL_FV(M)%CELL_VEL(1)=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,1)
          CELL_FV(M)%CELL_VEL(2)=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,2)
          CELL_FV(M)%CELL_VEL(3)=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,3) 
          CELL_FV(M)%CELL_TE    =TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,4)
          CELL_FV(M)%CELL_PHI   =TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,5) 
        END IF    
      ENDDO
      DEALLOCATE(TRAN)
        ...
        IF(FILE_EXIST)THEN
          ...
          DO M=1,TOTAL_CELL
            IF(CELL_FV(M)%CELL_ACTIVE.EQ.1)THEN
              ...
              CELL_FV(M)%CELL_PLM=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,1)  
              CELL_FV(M)%CELL_PMM=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,2) 
              CELL_FV(M)%CELL_PQN=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,3) 
              CELL_FV(M)%CELL_PNN=TRAN(CELL_SKIP(TOTAL_CELL_ACTIVE)+MM,4)    
            END IF    
          ENDDO      
          DEALLOCATE(TRAN)
